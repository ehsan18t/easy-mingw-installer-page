...
async function fetchRelease() {
  await fetch('https://api.github.com/repos/ehsan18t/easy-mingw-installer/releases/latest')
    ...
    .then(async data => {
      ...
      document.getElementById('btn-dl-64')?.setAttribute('href', data.assets.find((asset: any) => asset.name.includes('64-bit')).browser_download_url);
      document.getElementById('btn-dl-32')?.setAttribute('href', data.assets.find((asset: any) => asset.name.includes('32-bit')).browser_download_url);
